...
char getchar() {
    char ch;
    syscall(  SyscallGetc, (uint32_t)(&ch), (uint32_t)0, (uint32_t)0, (uint32_t)0 );
    return ch;
}

uint32_t getc(uint8_t* c){
    uint32_t status;
    syscall(  SyscallGetc, (uint32_t)(&status), (uint32_t)c, (uint32_t)0, (uint32_t)0 );
    return status;
}
...
void putc( uint8_t c ){
    ...
    syscall(  SyscallPutc, (uint32_t)c, (uint32_t)size, (uint32_t)colour, (uint32_t)0 );
}
...
void printf(const char *format, ...) {
    ...
    stdio_vcprintf((vcprintf_callback_t)putc, NULL, format, args);
    va_end(args);
}
...
int stdio_snprintf(char *buf, size_t size, const char *format, ...) {
    ...
    va_start(args, format);
    len = stdio_vsnprintf(buf, size, format, args);
    va_end(args);
 ...
 * the number.
 */
static int atoi(const char** ptr) {
    const char* s = *ptr;
    int i = 0;
    while(isdigit(*s)) {
    i = i * 10 + (*s++ - '0');
...
/* stdio_cprint_int - Convert integer to string
 ...
 * Returns nothing.
 */
void stdio_cprint_int(vcprintf_callback_t callback, void *state, uint64_t num,
...
/* stdio_vcprintf - Format a string and call callback for each char
 * @callback:   callback function to add char
...
void stdio_vcprintf(vcprintf_callback_t callback, void *state, const char* format,
    ...
    case 'i':
        ...
        stdio_cprint_int(callback, state, num, base, width, precision, flags);
        break;
    case 'p':
...
/* stdio_vsnprintf - Format a string and place it in a buffer
...
int stdio_vsnprintf(char* buf, size_t size, const char* format, va_list args) {
    ...
    stdio_vcprintf((vcprintf_callback_t)buffer_add, (void*)&state, format, args);
...
void stdio_cprintf(vcprintf_callback_t callback, void *state, const char* format,
    ...
    va_list args;
    va_start(args, format);
    stdio_vcprintf(callback, state, format, args);
    va_end(args);
}
